...
public class ImgController {
    ...
    public String fileUpload(@RequestParam(value = "file") MultipartFile file) {
        ...
        client.putObject(PutObjectArgs.builder().bucket(bucket).stream(go,go.available(),-1).object(fileName).contentType("image/png").build());
        return fileName;
    ...
    public void getPic(@RequestParam("w") int width,@RequestParam("h") int height,@PathVariable("bk") String bucket, @PathVariable("name") String name, HttpServletRequest request, HttpServletResponse response){
